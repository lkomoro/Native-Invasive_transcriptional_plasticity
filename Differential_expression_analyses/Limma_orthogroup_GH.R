...
##Figure 3: MA Plots (combined in adobe post-processing):####
#within Ds & HC between species:
#pdf(here("Differential_expression_analyses/temptreat_matrix_withinDs&HCDsMb_MAplotspanel.pdf"), width=12, height=6)
par(mfrow=c(2,4))
...
#within Mb
#pdf(here("Differential_expression_analyses/temptreat_matrix_withinMb_MAplotspanel.pdf"), width=12, height=6)
par(mfrow=c(2,4))
...
#note: the here function was throwing all sorts of weird errors below so I just scrapped it and checked the WD to make the appropriate relative file paths; I think there might be some interaction with tidyr?
library(plyr)
library(tidyr)
getwd()
annot.comb<-read.csv("Differential_expression_analyses/Annotations of Shared Orthogroups by Transcript_GHcp.csv")
annot.comb1<-annot.comb[,c(2,4)]
annot.unique<-(unique(annot.comb1))#removing redundancies
...
results<-read.table("Differential_expression_analyses/orthogroup.voom.GHcp.txt",header=TRUE)
...
#write.table(annot_compiled_contrasts, file = "Differential_expression_analyses/Orthogroup_annot_compiled_contrastslimma_rm2rds.txt", sep = "\t", quote = F, row.names = F)
