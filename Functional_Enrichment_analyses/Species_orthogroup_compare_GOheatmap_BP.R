...
# "The following code was written to illustrate the differential regulation of gene ontology biological processes by round and tubenose gobies exposed to 24 hours of increased and decreased temperature water. It was used to produce Figure 6 as drawn in Wellband and Heath (2017) Evolutionary Applications, 10: 563-576."

#Additional resources:
#https://bioconductor.org/packages/devel/bioc/vignettes/GOSemSim/inst/doc/GOSemSim.html
#http://bioconductor.org/packages/release/BiocViews.html#___OrgDb

#If want to compile custom package for your species:
...
library(here)
here()
setwd(here("./DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthogroup_GOheatmap_analyses"))
...
library(org.Dr.eg.db)#zebrafish database

# Read the list of GO terms to include in the heatmap
#Currently using results from Fisher's exact test, stricter threshold of p<=0.01; can make for relaxed threshold (p<0.05 or KS test results instead)
xx<-read.csv("../Orthogroup_TopGO_analyses/Combined_orthogroup_sigGOenrichFisherp0.01.csv")
...
#Can explore using other GO information from Danio rerio databases etc
goDr <- godata(OrgDb = org.Dr.eg.db, ont = "BP")
...
sum(is.na(sim))
#looked at using zebrafish as well, but has more NAs
simDr <- goSim(xx.1, xx.1, semData = goDr, measure = "Rel")
sum(is.na(simDr))
...
#NB this is definitely not ideal, but limited currently by databases and goal here is to look comparatively across the species so will achieve that.
...
#yy.1<-gather(yy,key="Treatment",value="pval",CTMax2.0.HC.Dspval,CTMax4.0.HC.Dspval,CTMax6.0.HC.Dspval,CTMax2.60.HC.Dspval,CTMax4.60.HC.Dspval,CTMax6.60.HC.Dspval,CTMax2.0.HC.Mbpval,CTMax4.0.HC.Mbpval,CTMax8.0.HC.Mbpval,CTMax12.0.HC.Mbpval,CTMax2.60.HC.Mbpval,CTMax4.60.HC.Mbpval,CTMax8.60.HC.Mbpval,CTMax12.60.HC.Mbpval) #keeping in case decide want this alternate order for graphic
...
#yy.1$Treatment<-factor(yy.1$Treatment, levels=c("CTMax2.0.Ds","CTMax4.0.Ds","CTMax6.0.Ds","CTMax2.60.Ds","CTMax4.60.Ds", "CTMax6.60.Ds", "CTMax2.0.Mb" ,"CTMax4.0.Mb","CTMax8.0.Mb","CTMax12.0.Mb" ,"CTMax2.60.Mb","CTMax4.60.Mb" ,"CTMax8.60.Mb", "CTMax12.60.Mb"))#keeping order in case want to reorder later for graphic
...
#Start here: 06-01-2020:
#sort and qualitatively analyze in excel to determine descriptive labels for each group-using term descriptions, AmiGO, and goSemSim output figures (to look for common type parents, etc.). Ideally in the future, I'd like to figure out a way to do this more quantitatively, i.e., roll up to parent terms but will work for goals here
