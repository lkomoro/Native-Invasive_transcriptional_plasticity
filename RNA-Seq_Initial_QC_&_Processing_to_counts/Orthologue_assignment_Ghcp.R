...
#Read in Data:####
orthologues<-read.table("./RNA-Seq_Initial_QC_&_Processing_to_counts/Orthologues_LMKworked.txt", header=T)
...
write.csv(Mb_ortho_long1,"./RNA-Seq_Initial_QC_&_Processing_to_counts//Mb_ortho_key_by_transcript.csv",row.names = FALSE)

#Menidia Part II: pull in counts per transcript table ####
Mb<-read.table("./RNA-Seq_Initial_QC_&_Processing_to_counts/menidia_combined_rawcounts.txt")#using the count tables with secondary reads removed
...
Mb_annot<-read.csv("./RNA-Seq_Initial_QC_&_Processing_to_counts/M_beryllina_fish_B2G_top_blasts.csv") #note this is just for sequence description, not GO IDs, see section below for GO IDs
...
write.csv(Ds_ortho_long1,"./RNA-Seq_Initial_QC_&_Processing_to_counts/Ds_ortho_key_by_transcript.csv",row.names = FALSE)

#Delta smelt Part II: pull in counts per transcript table ####
Ds<-read.table("./RNA-Seq_Initial_QC_&_Processing_to_counts/DS_combined_rawcounts.txt")
...
write.csv(SO.Ds,"./RNA-Seq_Initial_QC_&_Processing_to_counts/Ds_orthogroup_rawcounts.csv",row.names = FALSE)

#Bring in Annotation:
Ds_annot<-read.table("./RNA-Seq_Initial_QC_&_Processing_to_counts/delta_merged_final.annot.txt",header=FALSE, sep = "\t",fill = TRUE,na.strings=c("","NA"))#need fill=TRUE here because unequal columns in input file
...
Y$Species<-"Mb"
XY<-rbind(X,Y)
write.csv(XY, "./RNA-Seq_Initial_QC_&_Processing_to_counts/Annotations of Shared Orthogroups by Transcript.csv", row.names = F)

#Roll up to Orthogroup & combining across species:####
#note this is the brute force code I used before to roll up to gene, need to change to be loop so transferable
...
library(dplyr)
library(reshape2)
annot.comb<-read.csv("./RNA-Seq_Initial_QC_&_Processing_to_counts/Annotations of Shared Orthogroups by Transcript.csv")
counts.comb<-read.csv("./RNA-Seq_Initial_QC_&_Processing_to_counts/Orthogroup_count_speciescomb.csv")
...
Mb_GO<-read.delim("./RNA-Seq_Initial_QC_&_Processing_to_counts/M_beryllina_B2G_final.annot", header=F)

Ds_GO<-read.table("./RNA-Seq_Initial_QC_&_Processing_to_counts/delta_merged_final.annot.txt",header=FALSE, sep = "\t",fill = TRUE,na.strings=c("","NA"))#need fill=TRUE here because unequal columns in input file

Mb_key<-read.csv("./RNA-Seq_Initial_QC_&_Processing_to_counts/Mb_ortho_key_by_transcript.csv")
Mb_key<-Mb_key[,c(2,5)]
Ds_key<-read.csv("./RNA-Seq_Initial_QC_&_Processing_to_counts/Ds_ortho_key_by_transcript.csv")
Ds_key<-Ds_key[,c(2,5)]
...
write.csv(ZZ, "./RNA-Seq_Initial_QC_&_Processing_to_counts/GO terms of Shared Orthogroups by Transcript_raw.csv", row.names = F)
...
write.csv(ZZZ, "./RNA-Seq_Initial_QC_&_Processing_to_counts/GO terms of Shared Orthogroups by Transcript_nodups.csv", row.names = F)