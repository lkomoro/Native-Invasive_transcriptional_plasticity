...
counts<-read.csv(here("DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthofinder_analyses/analysis_2020/Orthogroup_count_speciescomb.csv"), row.names = 1)

#2b. Add in metadata:
#Can include as much companionate info as desired here to test different predictors, co-factors, confounding variables. But just (as always) remember that for inclusion in model, gets more challenging with more factors
metadata<-read.table(here("DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthogroup_DE_analyses/Combined.DsMb.metadata.txt"), header = T, stringsAsFactors = F)
...
#this is what Blythe used in EdgeR splittail analysis, I'm guessing because Ken indicated was interested in low expressed genes
...
#How did the filter threhold affect the distribution? Does the graph show that the low expressed genes are removed like they should be to reduce testing/FDR issues?  And/or need to adjust filtering thresholds further (can also check mean-variance graphs below)

#Extract for future graphing (nMDS):
z<-as.data.frame(lcpm)
rcpm <- cpm(counts2, log=FALSE)
zz<-as.data.frame(rcpm)
#write.csv(z, here("DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthogroup_DE_analyses/Temptime_splitanalysis/logCPM.orthogroup.limma.csv"),row.names=T)
#write.csv(zz,here("DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthogroup_DE_analyses/Temptime_splitanalysis/CPM.orthogroup.limma.csv"),row.names=T)

#3c.Inspect the relationships between samples using an exploratory multidimensional scaling (MDS) plot:
#(note-need to run #4 below first to create ed object)
png(here("DDIG_data&analyses/Transdecoder&Orthofinderwork/Orthogroup_DE_analyses/Temptime_splitanalysis/orthogroup_MDS.png"), height = 600, width = 500)
par(oma = c(5,0,0,0)) # expand outer margins to make room for legend
plotgroup <- interaction(metadata$Temp_treat_level, metadata$Time_point)
...
metadata$Condition<-factor(paste(metadata$Temp_time,metadata$Species,sep="."))#combining temp time, so can't look at interaction
